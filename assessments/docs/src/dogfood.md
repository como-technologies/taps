...
- **Producer side (this repo):** the `golden_export` integration test
  ...
  pins the YAML byte-for-byte against the vendored golden at
  `crates/amaker-core/tests/fixtures/golden-export.yaml`. Any change to
  the export shape fails CI here as a reviewable fixture diff — with no
  model in sight. See [Export Contract](./export-contract.md).
- **Consumer side (adroit):** adroit's own contract test pins
  `import --from-assessment` against a vendored copy of this app's export
  (`tests/fixtures/golden-assessment.yaml` in the adroit repo), so what
  its importer accepts and seeds can't drift silently either. That
  vendored copy was generated by an earlier revision of this exporter —
  it still validates against the current schema (the import seam holds),
  but re-vendoring it from the current export shape is pending on the
  adroit side.
