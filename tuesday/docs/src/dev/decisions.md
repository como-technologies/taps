...
corpus, managed with [adroit](https://github.com/como-technologies/adroit) —
...
`just adr-check` (part of `just ci`) runs exactly that seed-and-check and
skips with a notice when no adroit binary is available. New decisions are
authored as legacy-shape files in `docs/src/adr/` (see the corpus for the
house form) and validated the same way.
...
| ADR | Decision | Built? |
|---|---|---|
| ADR-0001 | Split tuesday into a `tuesday-core` library with web and CLI heads | Yes — the workspace (`crates/tuesday-core`, `-web`, `-cli`) is on `main` |
...
| ADR-0006 | Resolve cross-repo references through the suite's uniform resolution convention | Yes — `just adr-check` resolves the adroit binary env-first → sibling build → PATH → `.como/tools` cache |
