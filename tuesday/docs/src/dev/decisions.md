...
| ADR-0006 | Resolve cross-repo references through the suite's uniform resolution convention | Superseded (see the ADR) — ADR validation is the workspace-root `just adr-check` with the in-tree adroit; no resolution chain remains |
