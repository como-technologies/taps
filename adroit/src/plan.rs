//! Stored implementation plans — the pinned stored-plan contract.
//!
//! `adroit plan <ID> --save` persists an implementation plan *inside* the
//! decision page, as a marker-bracketed `## Implementation` section:
//!
//! ```text
//! ## Implementation
//!
...
//! ```
//!
//! The pure splice/extract engine lives here, terminal- and transport-free:
...
//! read path `plan <ID>` uses. The begin/end marker pair brackets the
//! section so the plan stays **opaque free-form markdown**: a plan
//! containing its own `## ` sub-headings round-trips verbatim (a single
//! begin marker with a stop-at-next-heading rule would truncate it).
//!
//! Only marker lines are adroit's. A section that is empty or nothing but an
//! italic `_…_` authoring prompt is a replaceable placeholder — [`splice`]
//! replaces it in place. A genuinely hand-written section is never touched:
//! [`has_hand_written_section`] lets the caller refuse to manage a plan
//! alongside one. Known limitation (documented, not defended): a plan whose
//! own text carries an `<!-- /adroit:plan -->` line is not representable
//! verbatim — extraction stops at the first end-marker line.
...
/// `(content_end, span_end)`. The end marker is authoritative (so a plan's
/// own `## ` sub-headings stay inside the section); only when it is lost
/// (hand-edited) does the section degrade to ending at the next `## `
...
/// The byte span of the managed section: from the `## Implementation`
/// heading (when it is the nearest non-blank line above the begin marker) or
/// the begin marker line, through the end-marker line inclusive. Without an
/// end marker (hand-edited), the span runs to the next `## ` heading or the
/// end of the body. `None` when the body has no begin-marker line.
...
/// `## ` heading or the end of the body. Callers use it only when there is
/// no marked span (the marked section's heading would match here too).
...
/// A section content adroit may replace: empty, or nothing but an italic
/// `_…_` authoring prompt (the placeholder a template ships with the
/// instruction to draft the plan later).
...
/// whose content is more than a template's placeholder prompt. The caller
/// refuses `--save` on such a page — adroit never overwrites or shadows
/// hand-written content.
pub fn has_hand_written_section(body: &str) -> bool {
    let span = marked_span(body);
    let hand_written = |cs: usize, ce: usize| !replaceable(&body[cs..ce]);
...
/// placeholder `## Implementation` section in place, else appends the
/// section at the end. Pure and idempotent for a marker-free `plan`
...
pub fn splice(body: &str, plan: &str) -> String {
    ...
    let span = marked_span(body).or_else(|| {
        unmarked_section_span(body)
            .and_then(|(s, cs, e)| replaceable(&body[cs..e]).then_some((s, e)))
    });
...
## Context and Problem Statement\n\nWe need a database.\n";
    ...
    fn extract_preserves_plan_sub_headings_verbatim() {
        // The end marker (not a stop-at-next-heading rule) bounds the
        // section, so a plan with its own `## ` sub-headings survives
        // byte-for-byte.
    ...
    fn splice_keeps_trailing_sections_after_the_managed_span() {
        ...
        // Replacing in place keeps the trailing `## Links` section after the
        // span.
    ...
    fn an_inline_marker_mention_is_not_a_section() {
        // Prose discussing the `<!-- adroit:plan -->` marker (e.g. a decision
        // about the plan contract itself) must not read as a stored plan:
        // only a line consisting solely of the marker counts.
    ...
    #[test]
    fn splice_replaces_a_template_placeholder_section_in_place() {
        // A prompt-only `## Implementation` ("draft it later") is
