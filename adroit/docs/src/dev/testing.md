...
| Cross-repo ingest contract | `import_golden_assessment_contract` in `tests/cli.rs`, over the canonical `contract/fixtures/golden-assessment.yaml` (workspace root) | `import` keeps seeding exactly the expected backlog from the `assessments` app's **real-exporter** golden export — the SAME file the exporter's byte-pin test reads, so export-contract drift fails one gate or the other, never silently | fast |
