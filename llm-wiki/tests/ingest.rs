...
fn ingest_validates_valid_page_and_commits() {
    ...
    write_page(&content_root, "concepts/foo.md", VALID_PAGE);
...
fn ingest_does_not_rewrite_file() {
    ...
    let before = fs::read_to_string(content_root.join("concepts/foo.md")).unwrap();
    ...
    let after = fs::read_to_string(content_root.join("concepts/foo.md")).unwrap();
...
fn ingest_folder_validates_all_md_recursively() {
    let dir = tempfile::tempdir().unwrap();
    let content_root = setup_repo(dir.path());
    write_page(&content_root, "concepts/a.md", VALID_PAGE);
    write_page(&content_root, "concepts/sub/b.md", VALID_PAGE);
...
fn ingest_folder_counts_assets() {
    let dir = tempfile::tempdir().unwrap();
    let content_root = setup_repo(dir.path());
    write_page(&content_root, "concepts/foo/index.md", VALID_PAGE);
    fs::write(content_root.join("concepts/foo/diagram.png"), b"fake").unwrap();
    fs::write(content_root.join("concepts/foo/config.yaml"), b"key: val").unwrap();
...
#[test]
fn ingest_rejects_path_outside_content_root() {
    let dir = tempfile::tempdir().unwrap();
    let content_root = setup_repo(dir.path());
...
fn changed_paths_skips_files_not_in_set() {
    ...
    let content_root = setup_repo(dir.path());
    write_page(&content_root, "concepts/a.md", VALID_PAGE);
    write_page(&content_root, "concepts/b.md", VALID_PAGE);
    write_page(&content_root, "concepts/c.md", VALID_PAGE);
    write_page(&content_root, "concepts/d.md", VALID_PAGE);
    write_page(&content_root, "concepts/e.md", VALID_PAGE);
