...
fn make_manager(dir: &Path) -> WikiIndexManager {
    let index_path = dir.join("index-store");
    WikiIndexManager::new("test", index_path)
}
...
fn open_searcher(mgr: &WikiIndexManager, _is: &IndexSchema) -> Searcher {
...
fn rebuild_report_fields() {
    let dir = tempfile::tempdir().unwrap();
    let content_root = setup_repo(dir.path());
    write_page(&content_root, "concepts/a.md", &concept_page("A", "body"));
    write_page(&content_root, "concepts/b.md", &concept_page("B", "body"));
    git::commit(dir.path(), "pages").unwrap();

    let mgr = make_manager(dir.path());
    let report = mgr
        .rebuild(&content_root, dir.path(), &schema(), &registry())
        .unwrap();
...
fn status_when_no_index() {
    ...
    let mgr = WikiIndexManager::new("test", dir.path().join("nonexistent"));
    let status = mgr.status(dir.path()).unwrap();
...
fn update_adds_new_page() {
    ...
    let mgr = make_manager(dir.path());
    mgr.rebuild(&content_root, dir.path(), &is, &reg).unwrap();
    ...
    let report = mgr
        .update(&content_root, dir.path(), None, &is, &reg)
        .unwrap();
...
fn update_deletes_removed_page() {
    ...
    fs::remove_file(content_root.join("concepts/gone.md")).unwrap();
    let report = mgr
        .update(&content_root, dir.path(), None, &is, &reg)
        .unwrap();
...
fn open_recovers_from_corruption() {
    ...
    let result = mgr.open(&is, Some((&content_root, dir.path(), &reg)));
    assert!(result.is_ok());
    assert!(mgr.searcher().is_ok());
}
...
fn update_refreshes_reader_immediately() {
    ...
    mgr.update(&content_root, dir.path(), None, &is, &reg)
        .unwrap();
