...
let wiki = engine.wiki(wiki_name)?;   // &Arc<WikiContext>
let searcher = wiki.index_manager.searcher()?;
// use wiki, searcher — engine guard stays alive for the block
...
The guard must outlive all borrows derived from it. `wiki` and `searcher`
...
`WikiIndexManager` (which has its own internal synchronisation), then
...
pub fn rebuild_index(&self, wiki_name: &str) -> Result<IndexReport> {
    let engine = self.state.read()
        .map_err(|_| anyhow::anyhow!("lock poisoned"))?;
    let wiki = engine.wiki(wiki_name)?;
    // WikiIndexManager.rebuild() does its own I/O; RwLock is still read-held
...
or hot-reload, when `WikiContext` itself needs to be inserted or replaced.
...
let wiki_name = { let e = self.state.read().unwrap(); e.wiki(name)?.name.clone() };
...
let state = GraphState::builder(cfg)
    .build_fn(move || {
        let wiki = engine.wiki(name)?;   // E0521: borrowed data escapes
        ...
    })
    .init()?;

// CORRECT — capture only owned/Arc data; re-acquire inside closure
let repo_root: PathBuf = { let e = self.state.read().unwrap(); e.wiki(name)?.repo_root.clone() };
let im = Arc::clone(&wiki.index_manager);
let state = GraphState::builder(cfg)
    .build_fn(move || {
        let searcher = im.searcher()...;
        let (tr, is) = wiki_builder::build_wiki(&repo_root, &tokenizer)?;
...
## `WikiContext` field access under the guard

`engine.wiki(name)` returns `&Arc<WikiContext>`. Fields on `WikiContext` are
...
let wiki = engine.wiki(wiki_name)?;                 // &Arc<WikiContext>
let im = Arc::clone(&wiki.index_manager);            // Arc clone — cheap
drop(engine);                                         // release guard
// im is now an independent Arc, safe to move into async or 'static closures
```

Plain fields like `type_registry` and `content_root` are not `Arc`-wrapped. To
capture them for `'static` use, clone them while the guard is held (if Clone),
or re-derive from disk inside the closure (if not Clone — e.g. `WikiTypeRegistry`).
...
These `WikiContext` fields do NOT need the `EngineState` write lock for
mutations — they use internal synchronisation:

| Field | Type | Sync mechanism |
|-------|------|----------------|
| `index_manager` | `Arc<WikiIndexManager>` | Internal `RwLock<IndexInner>` |
...
a `WikiContext` entry in `EngineState.wikis`.
