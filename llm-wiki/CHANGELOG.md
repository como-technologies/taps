...
- **Como schema library shipped in-engine** (#14) — `spaces create` now installs the five typed-page contract classes from kb-spec §2 alongside the bundled schemas: `decision` (moved from kb-spike, adroit-aligned status lifecycle, `superseded ⇒ superseded_by` conditional, `citations` declared for pinned `path@commit` refs), plus `guide`, `glossary-entry`, `worked-example`, and `plan`. All five set `additionalProperties: false`
- **`spaces create` provisions the admission model** (#14) — installs the two admission git hooks (`pre-commit`: `ingest . --dry-run`; `post-commit`: `ingest .`) with an ownership marker so user-owned hooks are never overwritten; writes `[validation] type_strictness = "strict"` and `[search.status]` weights for both status vocabularies into the generated `wiki.toml` (per-space, travels with the data); pins `index.auto_rebuild = true` (catch-up-on-read) in the registry config. Idempotent on every path. Hooks run only for real `git` invocations — libgit2 commits from the engine never execute them. Spec: `docs/specifications/tools/space-management.md`
